...
interface Props {
  ...
  searchQuery: string;
  onToggle: (record: Record) => Promise<void>;
    ...
    <div className="space-y-4">
      {filteredRecords.length === 0 ? (
        ...
        filteredRecords.map((record) => (
          <div
            key={record.id}
            className={`p-5 rounded-xl shadow-sm border transition ${record.is_complete ? "bg-gray-100 border-gray-200 opacity-60" : "bg-white border-blue-50"}`}
          >
            <div className="flex justify-between items-start">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold ${record.type === "MONEY" ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}
              >
